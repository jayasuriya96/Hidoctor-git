    ...
    if (expense_g[2].Data.length > 0 || expense_g[1].Data.length > 0) {
        ...
        if (expense_g[1].Data.length > 0) {
            ...
            if (expense_g[2].Data.length <= 0) { // if drafted data empty.                
                tableContent += "<tr>";
                tableContent += "<td><input type='text' id='txtExpense_" + i + "' class='autoexpense'  onclick= '$(this).select();' /><input type='hidden' id='hdnExpenseCode_" + i + "'/></td>";
        ...
        if (expense_g[2].Data.length > 0) {
            if (expense_g[1].Data.length > 0) { // if it has prefill data along with drafted.
                ...
                for (i = i; i <= count; i++) {
                    ...
                    if (!(expenseDraftCheck.length > 0)) {
                        tableContent += "<tr>";
                        tableContent += "<td><input type='text' id='txtExpense_" + j + "' value='" + expense_g[2].Data[k].ExpenseTypeName + "' class='autoexpense'  onclick= '$(this).select();'/><input type='hidden' id='hdnExpenseCode_" + j + "' value='" + expense_g[2].Data[k].ExpenseTypeCode + "'/></td>";
                ...
                for (var i = 1; i <= expense_g[2].Data.length; i++) {
                    tableContent += "<tr>";
                    tableContent += "<td><input type='text' id='txtExpense_" + i + "' value='" + expense_g[2].Data[i - 1].ExpenseTypeName + "' class='autoexpense'  onclick= '$(this).select();' /><input type='hidden' id='hdnExpenseCode_" + i + "' value='" + expense_g[2].Data[i - 1].ExpenseTypeCode + "'/></td>";
...
function fnCreateNewRowInExpense(e) {
    ...
    $(tdExpense).html("<input type='text' id='txtExpense_" + expenseRow + "' class='autoexpense'  onclick= '$(this).select();' /><input type='hidden' id='hdnExpenseCode_" + expenseRow + "'/>");
...
function fnRedirectToInstantReport() {
    var date = "", type = "", queryString = "";
    date = dcrDate.split('-')[1] + '/' + dcrDate.split('-')[2] + '/' + dcrDate.split('-')[0];
    ...
    var url = "http://" + domainName_g + "/ScreensReport/UserPerDay_New.aspx?";
    url += "usercode=" + $.trim(userCode) + " &dcrcode=" + dcrCode + "&dcrdate=" + date + "&source=dcr&status=" + dcrStatus + "&type=" + type + "&entity=" + escape(entity) + "&DCR_Flag=F&isrcpa=" + isrcpa + '&flag=' + flag_g;
    $('#main').load('../HiDoctor_Activity/DCRReport/Index/?dcrActualDate=' + dcrDate + '&flag=' + flag_g);
    HideModalPopup('dvLoading');
...
function fnRedirectToHeader() {
    if (isrcpa == "N") {
        $('#main').load('../HiDoctor_Activity/DCRHeader/Create/?dcrDate=' + dcrDate + '&dcrStatus=' + dcrStatus + '&isrcpa=' + isrcpa + '&source=TAB_STOCKIEST&flag=' + flag_g);
    }
    if (isrcpa == "R") {
        $('#main').load('../HiDoctor_Activity/DCRHeader/Create/?dcrDate=' + dcrDate + '&dcrStatus=' + status_g + '&isrcpa=Y&source=TAB_STOCKIEST&flag=' + flag_g);
    }
}
...
function fnRedirectToDoctorVisit() {
    ...
    if (isrcpa == "N") {
        $('#main').load('../HiDoctor_Activity/DCRDoctorVisit/Create/?status=' + dcrStatus + '&flagRCPA=N&accUsers=' + accRegions + '&cp='+escape(cpCode_g)+'&tp='+escape(tpCode_g)+'&dcrActualDate=' + dcrActualDate + '&category=' + escape(entity) + '&travelledkms=' + travelKm + '&source=TAB_STOCKIEST&flag=' + flag_g);
    }
    else if (isrcpa == "R") {
        $('#main').load('../HiDoctor_Activity/DCRDoctorVisit/Create/?status=' + dcrStatus + '&flagRCPA=R&accUsers=' + accRegions + '&cp=' + escape(cpCode_g) + '&tp=' + escape(tpCode_g) + '&dcrActualDate=' + dcrActualDate + '&category=' + escape(entity) + '&travelledkms=' + travelKm + '&source=TAB_STOCKIEST&flag=' + flag_g);
    }
}