...
var DieticianAPPReporting = {
    ...
    Init: function () {
        ...
        //  $("#Location").val(loc).attr('disabled', false);
        if (headerid != 0) {
        ...
        if (response != null && response.list.length > 0) {
            ...
            if (response.list.length == 0) {
                ...
                for (var i = 0; i < response.list.length; i++) {
                    _obj = {
                        id: $.trim(response.list[i].Region_Code),
                        name: $.trim(response.list[i].Region_Name + '-' + response.list[i].User_Name + '-' + response.list[i].Employee_Name + '-' + response.list[i].User_Type_Name)
                    };
                    data1.push(_obj);
                }
            }
            Users_g = data1;
            $('#dvusers').empty();
            $('#dvusers').html('<input type="text" id="ddluser" name="ddluser" class="form-control" maxlength="" />');
            var Users = new ej.dropdowns.ComboBox({
                ...
                onchange: DieticianAPPReporting.fngetdata,
            ...
            regioncode = selectedregionname;
            var msObject = document.getElementById("ddluser").ej2_instances[0];
            msObject.value = regioncode;
            msObject.enabled = false;
            //  $('select[name="ddluser"]').val(selectedregionname).attr('disabled', false);
            DieticianAPPReporting.fngetskudetails(k, regioncode);
            DieticianAPPReporting.fngetdocdetails(i, regioncode);
        } else {
            $('#dvusers').empty();
            $('#dvusers').html('<input type="text" id="ddluser" name="ddluser" class="form-control" maxlength="" />');
            var Users = new ej.dropdowns.ComboBox({
    ...
    fnadddoctor: function () {
        ...
        content += '<label for="DoctorName"><b>Doctor Name</b></label>';
        content += '<div class="col-sm-12" id="dvdoctor' + i + '">';
        content += '<input class="txtdoc" type="text" id="ddldoctor_' + i + '" tabindex="1"/>';
        ...
        content += '</div>';
        content += '<div class="form-group col-md-2">';
        content += '<label for="Prescription"><b>Prescription Value</b></label>';
        ...
        content += '<label for="Remarks"><b>Notes</b></label>';
    ...
    fngetdocdetails: function (val, regioncode) {
        doc_g = val;
        if (CompanyCode == "") {
        ...
        if (regioncode != "") {
            ...
            var Rcode = $('select[name="ddluser"]').val();
        }
        var details = DieticianAPPReporting.defaults.companyCode + '/' + Rcode
        // var details = DieticianAPPReporting.defaults.companyCode + '/' + regioncode;
        RPAREST.requestInvoke("DieticianReporting/GetDoctordetailsforDR", details, null, "GET", DieticianAPPReporting.fngetdocdetailsSuccessCallback, DieticianAPPReporting.fngetdocdetailsFailureCallback, null);
        ...
        if (response != null && response.list.length > 0) {
            ...
            var Users = new ej.dropdowns.MultiSelect({
                ...
                filtering: function (e) {
                    var dropdown_query = new ej.data.Query();
                    dropdown_query = (e.text !== '') ? dropdown_query.where('name', 'contains', e.text, true) : dropdown_query;
                    e.updateData(data1, dropdown_query);
                },
            });
    ...
    fngetskudetails: function (val, regioncode) {
        sku_g = val;
        if (CompanyCode == "") {
            var compcode = DieticianAPPReporting.defaults.companyCode;
        }
    ...
    fnsubmit: function (status) {
        ...
        var headerdet = {
            ...
            foruser: $('select[name="ddluser"]').val(),
        ...
        var Accompanist = [];
        var accomp = {};
        //if ($("select[name='additionalusers']").val() != undefined) {
        //    for (var i = 0; i < $("select[name='additionalusers']").val().length; i++) {
        //        accomp = {
        //            Accompanist_Code: $("select[name='additionalusers']").val()
        //        };
        ...
        for (var i = 0; i < $(".docgrid").length; i++) {
            ...
            if ($("select[name='ddldoctor_" + (i + 1) + "']").val() != null || $("#Speciality" + (i + 1) + "").val() != null || $("#Prescription" + (i + 1) + "").val() != "" || $("#remarks" + (i + 1) + "").val() != "") {
                ...
                if ($("select[name='ddldoctor_" + (i + 1) + "']").val() != null && ($("#Speciality" + (i + 1) + "").val() == -1 || $("#Speciality" + (i + 1) + "").val() == null)) {
                    ...
                    if ($("#doctorgrid" + (i + 1) + "").is(":visible") == true && ($("#ddldoctor_" + (i + 1) + "").val() != null || $("#ddldoctor_" + (i + 1) + "").val() != "")) {
                        docdetails.push(doc);
                    }
                }
            }
        ...
        for (var i = 0; i < $(".pregrid").length; i++) {
            ...
            //if ($("select[name='ddlsku" + (i + 1) + "']").val() == null) {
            //    swal({
            //        icon: "info",
            //        title: "Info",
            //        text: 'Please enter valid Product Name.Select from dropdown',
            //        button: "Ok",
            //    })
            //    return false;
            //}
    ...
    BindpostSuccessData: function (response) {
        ...
        if (response == "Success") {
            if (Status_G == 2) {
                ...
                swal({
                    text: "Once you submit you cannot edit.Are you sure you want to submit.",
                    buttons: true,
                    dangerMode: false,
                })
            .then((willDelete) => {
    ...
    fngetDieticiandataSuccessCallback: function (response) {
        if (response != null) {
            ...
            if (response.list.lstheaderdetails[0].status == 1) {
                $("#save").hide();
                $("#submit").hide();
                var regioncode = response.list.lstheaderdetails[0].User_Code;
                var msObject = document.getElementById("ddluser").ej2_instances[0];
                msObject.value = regioncode;
                // $('select[name="ddluser"]').val(response.list.lstheaderdetails[0].User_Code);
                ...
                //   $("#Location").val(response.list.lstheaderdetails[0].Location);
                // $("#ddlsubActivity").val(response.list.lstheaderdetails[0].Camp_Sub_Type);
