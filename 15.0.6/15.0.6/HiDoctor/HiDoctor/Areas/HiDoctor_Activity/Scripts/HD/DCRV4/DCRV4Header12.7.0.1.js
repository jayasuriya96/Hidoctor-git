...
var prefill_g = "", prefillHop_g = "", sfc_g = "", atten_g = "", cp_g = "", cpHop_g = "", distEdit = "", docAccomp_g = "", TP_g = "", TPSFC_g = ""; AccompMsg = "", MasterDistance = 0, popupdistance = "";
var Acc_Id_g = [];
...
function fnBindCategory(category) {
    if (category.length > 0) {
        ...
        // fngetCategorySetting(category[0].value);
    }
}
debugger;
        ...
        $.ajax({
            ...
            success: function (jsHeaderData) {
                ...
                for (var i = 0; i < Header_g[0].Data[0].Data.length; i++) {
                    accompanist += "{label:" + '"' + "" + Header_g[0].Data[0].Data[i].Accompanist_Name + "" + '",' + "value:" + '"' + "" + Header_g[0].Data[0].Data[i].Accompanist_Region_Code + "" + '",' + "child:" + '"' + "" + Header_g[0].Data[0].Data[i].Child_Count + "" + '"' + "}";
                ...
                GetDCRFreezeStatus();
                fnFreezeDCR();
                setting_prefill = Header_g[0].Data[4].Data;
            },
            error: function (e) {
...
///  Work Area/////////
function fnGetWorkAreas(regionCode, json) {
    debugger
    var beatPlan_SFCValidatPriv_g = fnGetPrivilegeValue("SFC_VALIDATION", "");
    var value = 0;
    var workCategoryName = $('#ddlCategory').val();
    var workCategory = $("#ddlCategory option:selected").text();
    ...
    $('.autoAccompanist').each(function () {
        debugger;
        var rowId = $(this)[0].id.split('_')[1];
        if ($('#hdnAccompanistCode_' + rowId).val() != "") {
            lstAccompanistsRegions.push($('#hdnAccompanistCode_' + rowId).val());
        }
    });
    lstAccompanistsRegions.push(RegionCode);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var lstWorkAreas = [];
            var data1 = new Array();
            var workCategoryName = $('#ddlCategory option:selected').text();
            ...
            if (json != '' && json != undefined) {
                var lstWorkAreaSelected = json;
                for (var k = 0; k < json.length; k++) {
                    var Warea = $.grep(response, function (v) {
                        return v.Work_Area == json[k];
                    });
                    if (Warea.length == 0) {
                        var obj = {
                            Region_Code: '',
            ...
            if (response != null && response.length > 0) {
                ...
                $('#Work_Place').empty();
                $('#wcat').html('<input type="text" id="Work_Place" name="Work_Place" class="form-control" />');
                ...
                WorkAreas.appendTo('#Work_Place');
            }
            else {
                $('#Work_Place').empty();
                $('#wcat').html('<input type="text" id="Work_Place" name="Work_Place" class="form-control" maxlength="" />');
                var WorkAreas = new ej.dropdowns.MultiSelect({
                    // set the countries data to dataSource property
                    dataSource: data1,
                    // map the appropriate columns to fields property
...
function fnPrefillAttendance() {
    ...
    if (atten_g[0].Data_From != 'WA') {
        ...
        lstWPArr = [];
        if (atten_g[0].Work_Place.lastIndexOf(',') > -1) {
            lstWPArr = atten_g[0].Work_Place.split(',');
        }
        else {
            lstWPArr.push(atten_g[0].Work_Place);
        }
        //$("select[name='Work_Place']").val();
    }
    else if (TP_g != "" && dcrStatus == "1") { // WA TP data should be prefilled
        $("#ddlCategory").val(TP_g[0].Category);
        lstWPArr.push(TP_g[0].Work_Place);
        //$("select[name='Work_Place']").val(TP_g[0].Work_Place);
    }
    ...
    fnGetWorkAreas(RegionCode, lstWPArr)
    //var msObject = document.getElementById('Work_Place').ej2_instances[0];
    //msObject.value = lstWPArr;
    fnSetIntermediatePrivilege();
...
function fnPrefillData() {
    debugger;
    popupdistance = prefill_g[0].Distance;
    ...
    if (prefill_g[0].Data_From != 'WA') {
        ...
        debugger;
        var work_Area_g = [];
        var workArea = prefill_g[0].Work_Place;
        ...
        fnGetWorkAreas(RegionCode, work_Area_g)
        ...
        var work_Area_g = [];
        var workArea = TP_g[0].Work_Place;
        if (workArea.lastIndexOf(',') == -1) {
            work_Area_g.push(workArea);
        }
        else {
...
function fnCreateAccompanistTable(isPrefill) {
    ...
    Show_outside_accompanist_privilege = fnGetPrivilegeValue("ALLOW_OTHER_HIERARCHY_IN_ACCOMPANIST_IN_APP", "NO");
    debugger;
    if (flag_g != "A") {
        var tableContent = "";
        ...
        if (isPrefill == "Y" && prefill_g != "") {
            ...
            for (var i = 1; i <= 4; i++) {
                if (i == 1 && prefill_g[0].Acc1_Name != "" && prefill_g[0].Acc1_Name != 'null') {
                    accObj.push({
                        accName: prefill_g[0].Acc1_Name, accCode: prefill_g[0].Acc1_Code, accOnlyDoc: prefill_g[0].Acc1_Only_For_Doctor, st: prefill_g[0].Acc1_Start_Time, et: prefill_g[0].Acc1_End_Time, accMode: prefill_g[0].Acc1_Mode_Of_Entry
                    });
                }
                else if (i == 2 && prefill_g != "" && prefill_g[0].Acc2_Name != "" && prefill_g[0].Acc2_Name != 'null') {
                    accObj.push({
                        accName: prefill_g[0].Acc2_Name, accCode: prefill_g[0].Acc2_Code, accOnlyDoc: prefill_g[0].Acc2_Only_For_Doctor, st: prefill_g[0].Acc2_Start_Time, et: prefill_g[0].Acc2_End_Time, accMode: prefill_g[0].Acc2_Mode_Of_Entry
                    });
                }
                else if (i == 3 && prefill_g != "" && prefill_g[0].Acc3_Name != "" && prefill_g[0].Acc3_Name != 'null') {
                    accObj.push({
                        accName: prefill_g[0].Acc3_Name, accCode: prefill_g[0].Acc3_Code, accOnlyDoc: prefill_g[0].Acc3_Only_For_Doctor, st: ((prefill_g[0].Acc3_Start_Time == null) ? null : prefill_g[0].Acc3_Start_Time.split('_')[0]), et: ((prefill_g[0].Acc3_Start_Time == null) ? null : prefill_g[0].Acc3_Start_Time.split('_')[1]), accMode: prefill_g[0].Acc3_Mode_Of_Entry
                    });
                }
                else if (i == 4 && prefill_g != "" && prefill_g[0].Acc4_Name != "" && prefill_g[0].Acc4_Name != 'null') {
                    accObj.push({
                        accName: prefill_g[0].Acc4_Name, accCode: prefill_g[0].Acc4_Code, accOnlyDoc: prefill_g[0].Acc4_Only_For_Doctor, st: ((prefill_g[0].Acc4_Start_Time == null) ? null : prefill_g[0].Acc4_Start_Time.split('_')[0]), et: ((prefill_g[0].Acc4_Start_Time == null) ? null : prefill_g[0].Acc4_Start_Time.split('_')[1]), accMode: prefill_g[0].Acc4_Mode_Of_Entry
                    });
                }
            }
            ...
            for (var k = 0; k < accObj.length; k++) {
                ...
                fnindependentchk((k + 1), accObj[k].accCode);
            ...
            if (k == 4) {
                ...
                tableContent += "<tr>";
                tableContent += "<td>" + (k + 1) + "</td>";
                tableContent += "<td style='width: 200px;'><input type='text' id='txtAccompanist_" + (k + 1) + "'  class='autoAccompanist'  onclick= '$(this).select();'/><input type='hidden' id='hdnAccompanistCode_" + (k + 1) + "'/><input type='hidden' id='hdnAccompMode_" + (k + 1) + "'/>";
                if (Show_outside_accompanist_privilege == "YES") {
                    tableContent += "<span id='popup_" + (k + 1) + "' class='accPopup'>&nbsp&#62&#62</span></td>";
                }
                else {
                ...
                for (var i = 1; i <= 2; i++) {
                    ...
                    tableContent += "<td style='width: 200px;'><input type='text' id='txtAccompanist_" + i + "'  class='autoAccompanist'  onclick= '$(this).select();'/><input type='hidden' id='hdnAccompanistCode_" + i + "'/><input type='hidden' id='hdnAccompMode_" + i + "'/>";
                    if (Show_outside_accompanist_privilege == "YES") {
                        tableContent += "<span id='popup_" + i + "' class='accPopup'>&nbsp&#62&#62</span></td>";
                    }
                    else {
                        tableContent += "</td>";
                    }
...
function fnCreateIntermediatePlaceTable(isPrefill) {
    ...
    var distanceEditJson;
    if (distEdit != "" && distEdit.length > 0) {

        distanceEditJson = jsonPath(distEdit, "$.[?(@.Category=='" + $("#ddlCategory").val().toString() + "' & @.SFC_Type!='E' & @.Is_Prefill!='N')]");
    ...
    if (dcrStatus == "1" && source != "TAB" && isPrefill == "Y") {
        ...
        if (intermediateNeed == "NO" && $("#ddlCategory :selected").text().toUpperCase() == "HQ") {
            if (tpSFCPrefill != "" && tpSFCPrefill.length > 0) {
                ...
                if (distanceEditJson != undefined) {
                    if (distanceEditJson.length > 0) {
                        if (tpSFCPrefill[0].Distance_Fare_Code != "" && distanceEditJson[0].Distance_Edit == "R") {
                            tableContent += "<td><input type='text' id='txtDistance_1' " + style + "  disabled='disabled' class='numeric fromPlace' value='" + tpSFCPrefill[0].Distance + "' onclick= '$(this).select();' /><input type='hidden' value='" + tpSFCPrefill[0].Distance_Fare_Code + "' id='hdnDistanceFareCode_1'/><input type='hidden' value='" + tpSFCPrefill[0].Distance + "' id='hdnMasterDistance_1'/>";
                        }
                        else {
                            tableContent += "<td><input type='text' id='txtDistance_1' " + style + "  class='numeric fromPlace' value='" + tpSFCPrefill[0].Distance + "' onclick= '$(this).select();' /><input type='hidden' value='" + tpSFCPrefill[0].Distance_Fare_Code + "' id='hdnDistanceFareCode_1'/> <input type='hidden' value='" + tpSFCPrefill[0].Distance + "' id='hdnMasterDistance_1'/>";
                        }
                    }
                    else {
                ...
                tableContent += "<td><input type='text' id='txtDistance_1'  " + style + "  class='numeric fromPlace' onclick= '$(this).select();' /><input type='hidden' id='hdnDistanceFareCode_1'/><input type='hidden' id='hdnMasterDistance_1'/><input type='hidden' id='hdnRouteWay_1'/><input type='hidden' id='hdnSFCRegion_1'/><input type='hidden' id='hdnSFCVersion_1'/><input type='hidden'  id='hdnSFCCategory_1'/> <input type='hidden' value='1' id='hdnIs_TP_SFC_1'/></td>";
                tableContent += "</tr>";
                interRow = 2;
            }
        }
            ...
            if (tpSFCPrefill != "" && tpSFCPrefill.length > 0) {
                ...
                if (intermediateNeed == "YES") {
                    for (var k = 1; k <= count; k++) {
                        if (k == count && isRouteCompleteinTP && intermediateNeed == "No") { // TP data has route completed data
                            ...
                            tableContent += "<td><label id='lblDistance_Auto'>" + tpSFCPrefill[k - 1].Distance + "</label><input type='hidden' id='hdnDistanceFareCode_Auto' value='" + sfcCode + "'/><input type='hidden' id='hdnMasterDistance_Auto' value='" + tpSFCPrefill[k - 1].Distance + "'/></td>";
                            ...
                            if (tpSFCPrefill[i - 1].Distance_Fare_Code != "") {
                                tableContent += "<td><input type='text'  disabled='disabled'  id='txtDistance_" + i + "' " + style + " value='" + tpSFCPrefill[i - 1].Distance + "'  class='numeric fromPlace' onclick= '$(this).select();' /><input type='hidden' value='" + tpSFCPrefill[i - 1].Distance_Fare_Code + "' id='hdnDistanceFareCode_" + i + "'/><input type='hidden' value='" + tpSFCPrefill[i - 1].Distance + "' id='hdnMasterDistance_" + i + "'/>";
                            } else {
                                tableContent += "<td><input type='text' id='txtDistance_" + i + "' " + style + " value='" + tpSFCPrefill[i - 1].Distance + "'  class='numeric fromPlace' onclick= '$(this).select();' /><input type='hidden' value='" + tpSFCPrefill[i - 1].Distance_Fare_Code + "' id='hdnDistanceFareCode_" + i + "'/><input type='hidden' value='" + tpSFCPrefill[i - 1].Distance + "' id='hdnMasterDistance_" + i + "'/>";
                            }
                    ...
                    if ($("#ddlCategory :selected").text().toUpperCase() != "HQ") {
                        if ($.inArray($("#ddlCategory :selected").text(), sfcValidation) > -1) { // SFC_VALIDATION privilege check.
                            if (($.inArray($("#ddlCategory :selected").text(), hopRouteCategory) > -1)) {
                                ...
                                tableContent += "<td><label id='lblDistance_Auto'>" + tpSFCPrefill[i - 1].Distance + "</label><input type='hidden' id='hdnDistanceFareCode_Auto' value='" + sfcCode + "'/><input type='hidden' id='hdnMasterDistance_Auto' value='" + tpSFCPrefill[i - 1].Distance + "'/></td>";
                ...
                if ($("#ddlCategory :selected").text() != "HQ" && intermediateNeed == "YES") {
                    ...
                    tableContent += "<td><input type='text' id='txtDistance_" + i + "' " + style + "   class='numeric fromPlace' onclick= '$(this).select();' /><input type='hidden' id='hdnDistanceFareCode_" + i + "'/><input type='hidden' id='hdnMasterDistance_" + i + "'/><input type='hidden' id='hdnRouteWay_" + i + "'/>";
        ...
        if (distanceEditJson != undefined) {
            if (distanceEditJson.length > 0) {
                if (prefill_g[0].Distance_Fare_Code != "" && distanceEditJson[0].Distance_Edit == "R") {
                    tableContent += "<td><input type='text' disabled='disabled' id='txtDistance_1' " + style + "  class='numeric fromPlace' value='" + prefill_g[0].Distance + "' onclick= '$(this).select();' /><input type='hidden' value='" + prefill_g[0].Distance_Fare_Code + "' id='hdnDistanceFareCode_1'/><input type='hidden' value='" + prefill_g[0].Distance + "' id='hdnMasterDistance_1'/><input type='hidden' value='" + prefill_g[0].Route_Way + "' id='hdnRouteWay_1'/>";
                } else {
                ...
                if (isRouteCompleteFlag == true && intermediateNeed == "NO") {
                    ...
                    */
                    ...
                    if (SFCWithoutRouteComplete != null && SFCWithoutRouteComplete && SFCWithoutRouteComplete.length > 0) {
                        ...
                        tableContent += "<td><input type='text' id='txtDistance_1' " + style + " value='" + SFCWithoutRouteComplete[0].Distance + "'  class='numeric fromPlace' onclick= '$(this).select();' /><input type='hidden' value='" + SFCWithoutRouteComplete[0].Distance_Fare_Code + "' id='hdnDistanceFareCode_1'/><input type='hidden' value='" + SFCWithoutRouteComplete[0].Distance + "' id='hdnMasterDistance_1'/><input type='hidden'  value='" + SFCWithoutRouteComplete[0].Route_Way + "' id='hdnRouteWay_1'/>";
                    ...
                    if (routeCompleteJson != false && routeCompleteJson !== undefined && routeCompleteJson.length > 0) {
                        ...
                        tableContent += "<td><label id='lblDistance_Auto'>" + routeCompleteJson[0].Distance + "</label><input type='hidden' id='hdnDistanceFareCode_Auto' value='" + routeCompleteJson[0].Distance_Fare_Code + "'/><input type='hidden' id='hdnMasterDistance_Auto' value='" + routeCompleteJson[0].Distance + "'/><input type='hidden' id='hdnRouteWay_Auto'  value='" + routeCompleteJson[0].Route_Way + "'/>";
                        ...
                        tableContent += "<td><label id='lblDistance_Auto'>" + SFCWithoutRouteComplete[0].Distance + "</label><input type='hidden' id='hdnDistanceFareCode_Auto' value='" + SFCWithoutRouteComplete[0].Distance_Fare_Code + "'/> <input type='hidden' id='hdnMasterDistance_Auto' value='" + SFCWithoutRouteComplete[0].Distance + "'/>";
                    ...
                    if (intermediateNeed == "YES") {
                        for (var i = 1; i <= count; i++) {
                            ...
                            if (prefillHop_g[i - 1].Distance_Fare_Code != "") {
                                tableContent += "<td><input type='text' disabled='disabled' id='txtDistance_" + i + "' " + style + " value='" + prefillHop_g[i - 1].Distance + "'  class='numeric fromPlace' onclick= '$(this).select();' /><input type='hidden' value='" + prefillHop_g[i - 1].Distance_Fare_Code + "' id='hdnDistanceFareCode_" + i + "'/><input type='hidden' value='" + prefillHop_g[i - 1].Distance + "' id='hdnMasterDistance_" + i + "'/><input type='hidden'  value='" + prefillHop_g[i - 1].Route_Way + "' id='hdnRouteWay_" + i + "'/>";
                            } else {
                                tableContent += "<td><input type='text' id='txtDistance_" + i + "' " + style + " value='" + prefillHop_g[i - 1].Distance + "'  class='numeric fromPlace' onclick= '$(this).select();' /><input type='hidden' value='" + prefillHop_g[i - 1].Distance_Fare_Code + "' id='hdnDistanceFareCode_" + i + "'/><input type='hidden' value='" + prefillHop_g[i - 1].Distance + "' id='hdnMasterDistance_" + i + "'/><input type='hidden'  value='" + prefillHop_g[i - 1].Route_Way + "' id='hdnRouteWay_" + i + "'/>";
                            }
        ...
        if (distanceEditJson != false && distanceEditJson != undefined) {
            if (distanceEditJson.length > 0) {
                if (cpJson[0].Distance_Fare_Code != "" && distanceEditJson[0].Distance_Edit == "R") {
                    tableContent += "<td><input type='text' disabled='disabled' id='txtDistance_1' " + style + "  class='numeric fromPlace' value='" + cpJson[0].Distance + "' onclick= '$(this).select();' /><input type='hidden' value='" + cpJson[0].Distance_Fare_Code + "' id='hdnDistanceFareCode_1'/><input type='hidden' value='" + cpJson[0].Distance + "' id='hdnMasterDistance_1'/><input type='hidden' value='" + cpJson[0].Route_Way + "' id='hdnRouteWay_1'/>";
                } else {
                    tableContent += "<td><input type='text' id='txtDistance_1' " + style + "  class='numeric fromPlace' value='" + cpJson[0].Distance + "' onclick= '$(this).select();' /><input type='hidden' value='" + cpJson[0].Distance_Fare_Code + "' id='hdnDistanceFareCode_1'/><input type='hidden' value='" + cpJson[0].Distance + "' id='hdnMasterDistance_1'/><input type='hidden' value='" + cpJson[0].Route_Way + "' id='hdnRouteWay_1'/>";
                }
            }
        ...
        if (intermediateNeed != "NO" && $("#ddlCategory :selected").text() != "HQ") {
            ...
            tableContent += "<td><input type='text' id='txtDistance_2' " + style + "   class='numeric fromPlace' onclick= '$(this).select();' /><input type='hidden' id='hdnDistanceFareCode_2'/><input type='hidden' id='hdnMasterDistance_2'/><input type='hidden' id='hdnRouteWay_2'/>";
        ...
        var work_Area_g = [];
        var workArea = cpJson[0].Work_Place;
        if (workArea.lastIndexOf(',') == -1) {
            work_Area_g.push(workArea);
        }
        else {
        ...
        if (cpJson.length > 0) {
            ...
            if (intermediateNeed == "YES") {
                for (var i = 0; i < cpJson.length; i++) {
                    if (cpJson[i].From_Place != "") {
                        ...
                        if (i == (cpJson.length - 1) && isRouteCompleteinCP == true && intermediateNeed == "NO") { // CP data has route completed data
                            ...
                            tableContent += "<td><label id='lblDistance_Auto'>" + cpJson[i].Distance + "</label><input type='hidden' id='hdnDistanceFareCode_Auto' value='" + cpJson[i].Distance_Fare_Code + "'/><input type='hidden' id='hdnMasterDistance_Auto' value='" + cpJson[i].Distance + "'/><input type='hidden' id='hdnRouteWay_Auto'  value='" + cpJson[i].Route_Way + "'/>";
                            ...
                            if (cpJson[i].Distance_Fare_Code != "") {
                                ...
                                tableContent += "<td><input type='text' disabled='disabled'  id='txtDistance_" + j + "' " + style + "  class='numeric fromPlace' value='" + cpJson[i].Distance + "' onclick= '$(this).select();' /><input type='hidden' value='" + cpJson[i].Distance_Fare_Code + "' id='hdnDistanceFareCode_" + j + "'/><input type='hidden' value='" + cpJson[i].Distance + "' id='hdnMasterDistance_" + j + "'/><input type='hidden' value='" + cpJson[i].Route_Way + "' id='hdnRouteWay_" + j + "'/>";
                            } else {

                                tableContent += "<td><input type='text' id='txtDistance_" + j + "' " + style + "  class='numeric fromPlace' value='" + cpJson[i].Distance + "' onclick= '$(this).select();' /><input type='hidden' value='" + cpJson[i].Distance_Fare_Code + "' id='hdnDistanceFareCode_" + j + "'/><input type='hidden' value='" + cpJson[i].Distance + "' id='hdnMasterDistance_" + j + "'/><input type='hidden' value='" + cpJson[i].Route_Way + "' id='hdnRouteWay_" + j + "'/>";
                            }
                ...
                if (isRouteComplete == true) {
                    ...
                    tableContent += "<td><label id='lblDistance_Auto'>" + cpJson[0].Distance + "</label><input type='hidden' id='hdnDistanceFareCode_Auto' value='" + cpJson[0].Distance_Fare_Code + "'/><input type='hidden' id='hdnMasterDistance_Auto' value='" + cpJson[0].Distance + "'/><input type='hidden' id='hdnRouteWay_Auto'  value='" + cpJson[0].Route_Way + "'/>";
            ...
            if (intermediateNeed == "YES") {
                ...
                tableContent += "<td><input type='text' id='txtDistance_" + j + "'  " + style + "  class='numeric fromPlace' onclick= '$(this).select();' /><input type='hidden' id='hdnDistanceFareCode_" + j + "'/><input type='hidden' id='hdnMasterDistance_" + j + "'/><input type='hidden' id='hdnRouteWay_" + j + "'/>";
        ...
        //currRegionName
        //currentRegion
        tableContent += "<tr>";//" + currRegionName + "
        tableContent += "<td><input type='text' id='txtFromPlace_1'  class='autoIntermediate fillDist changeEvent' value=''   /><input type='hidden' value='' id='hdnFromPlace_1'/></td>";
        tableContent += "<td><input type='text' id='txtToPlace_1' onblur='fnTxtToPlaceOnBlur()' class='autoIntermediate fillDist fromPlace newInter' value=''  onclick= '$(this).select();' /><input type='hidden' value='' id='hdnToPlace_1'/></td>";
        tableContent += "<td><input type='text' id='txtTravelMode_1'   class='autotravel fromPlace' onclick= '$(this).select();'/><input type='hidden'  id='hdnTravelMode_1'/></td>";
        tableContent += "<td><input type='text' id='txtDistance_1' " + style + "   class='numeric fromPlace' onclick= '$(this).select();' /><input type='hidden' id='hdnDistanceFareCode_1'/><input type='hidden' id='hdnMasterDistance_1'/><input type='hidden' id='hdnRouteWay_1'/>";
...
function fnTxtToPlaceOnBlur() {
    debugger;
    if ($('select[name="Work_Place"]').val() == null) {
        var json = [];
        var place = $('#txtToPlace_1').val();
        json.push(place);
        if ($('#txtToPlace_1').val() != '')
            fnGetWorkAreas(RegionCode, json);
    }
}
function fnCreateNewRowInAccompanist(e) {
    ...
    if (accompRow <= 4) {
        ...
        if (Show_outside_accompanist_privilege == "YES") {
            $(tdAccompanist).html("<input type='text' id='txtAccompanist_" + accompRow + "'  class='autoAccompanist'  onclick= '$(this).select();' onblur='fnindependentchk(" + accompRow + ",'');'/><input type='hidden' id='hdnAccompanistCode_" + accompRow + "' /><input type='hidden' id='hdnAccompMode_" + accompRow + "'/><span id='popup_" + accompRow + "' class='accPopup'>&nbsp&#62&#62</span>");
        }
        else {
            $(tdAccompanist).html("<input type='text' id='txtAccompanist_" + accompRow + "'  class='autoAccompanist'  onclick= '$(this).select();' onblur='fnindependentchk(" + accompRow + ",'');'/><input type='hidden' id='hdnAccompanistCode_" + accompRow + "' /><input type='hidden' id='hdnAccompMode_" + accompRow + "'/>");
        }
    ...
    if ($.trim($(e).val()).length > 0) {
        if (intermediateNeed != "NO" && $("#ddlCategory :selected").text().toUpperCase() != "HQ") {
            ...
            if ($("#txtFromPlace_" + newRowId).length > 0) {
                ...
                $(tdDistance).html("<input type='text' id='txtDistance_" + interRow + "' " + style + "  class='numeric fromPlace' onclick= '$(this).select();' /><input type='hidden' id='hdnDistanceFareCode_" + interRow + "'/><input type='hidden' id='hdnMasterDistance_" + interRow + "'/><input type='hidden' id='hdnRouteWay_" + interRow + "'/><input type='hidden' id='hdnSFCRegion_" + interRow + "'/><input type='hidden' id='hdnSFCVersion_" + interRow + "'/><input type='hidden' id='hdnSFCCategory_" + interRow + "'/><span id='spnDelSFCRow_" + interRow + "' onclick='fnDeleteSFCRow(this)' style='font-weight:bold;cursor:pointer'>&nbsp;&nbsp;&nbsp;X&nbsp;&nbsp;</span>");
...
function fnHOPRouteComplete(rCnt) {
    ...
    if ($.inArray($("#ddlCategory :selected").text(), sfcValidation) > -1 && intermediateNeed == "NO") {
        if ($("#txtToPlace_" + rCnt).val() != "" && $("#txtFromPlace_" + rCnt).val() != "" && $("#txtTravelMode_" + rCnt).val() != "") {
            ...
            $(tdDistance).html("<label id='lblDistance_Auto'></label><input type='hidden' id='hdnDistanceFareCode_Auto'/><input type='hidden' id='hdnMasterDistance_Auto'/><input type='hidden' id='hdnRouteWay_Auto'/><input type='hidden' id='hdnSFCRegion_Auto'/><input type='hidden' id='hdnSFCVersion_Auto'/><input type='hidden' id='hdnSFCCategory_Auto'/>");
            ...
            if (!(distanceJson === undefined) && distanceJson.length > 0) {
                if (distanceJson.length > 1) {
                    ...
                    $("#hdnMasterDistance_Auto").val(distanceJson[0].Distance);
...
function fnAccompanistEventBinder() {
    ...
    debugger;
    $(".autoAccompanist").keyup(function () {
        fnCreateNewRowInAccompanist(this.id);
    });
    $(".autoAccompanist").dblclick(function () {
        fnCreateNewRowInAccompanist(this.id);
    });
    ...
    $(".autoAccompanist").focus(function () {
        fnRemoveSFC(this);
    });

    // to check 1st text box empty.
    $(".accomp").blur(function () {
        if ($(this).val() != "") { return fnCheckAccompanist(this.id) }
    });

    $(".accompchk").click(function () {
        if ($(this).attr('checked')) {
            return fnCheckAccompanist(this.id)
        }
    });
...
function fnIntermediateEventBinder() {
    ...
    $(".newInter").dblclick(function () {
        fnCreateNewRowInIntermediate(this);
    });
    $(".newInter").blur(function () {
        fnCreateNewRowInIntermediate(this); fnFillDistanceTravelMode(this.id);
    });

    // to fill distance ,travel mode in to_place blur event.
    $(".fillDist").blur(function () {
        fnFillDistanceTravelMode(this.id);
    });
    $(".autotravel").blur(function () {
        fnFillDistanceTravelMode(this.id);
    });
...
function fnSetIntermediatePrivilege() {
    ...
    var Category_Code = $('#ddlCategory').val();
    fngetCategorySetting(Category_Code);
    //work_Area_g = [];
    // fnGetWorkAreas(RegionCode);
}
...
function fnFillDistanceTravelMode(id) {
    ...
    var TravelDistanceEdit = $.grep(distEdit, function (ele) {
        return ele.SFC_Type != "E";
    });

    var DistanceeditAllow;
    if (TravelDistanceEdit.length > 0) {
        DistanceeditAllow = TravelDistanceEdit[0].Distance_Edit;
    }
    ...
    if ($("#txtFromPlace_" + rCnt).val() != "" && $("#txtToPlace_" + rCnt).val() != "" && $("#txtTravelMode_" + rCnt).val() != "") {
        if (sfc_g != "") {
            ...
            if (distanceJson != null && distanceJson.length > 0) {
                ...
                $("#hdnMasterDistance_" + rCnt).val(cur_sfc[0].Distance);
                // Change attribute Disabled to Distance based on hdnDistanceFareCode
                if ($("#hdnDistanceFareCode_" + rCnt).val() != "" && DistanceeditAllow != "A") {
            ...
            if (sfc_g != "") {
                ...
                if (distanceJson.length > 0 && distanceJson != null) {//No valus found - Clear Textbox
                    ...
                    if (f_distanceJson.length > 1) { // if more than one sfc data available show pop up
                        ...
                        $("#hdnMasterDistance_" + rCnt).val(cur_sfc.Distance);
                        // Change attribute Disabled to Distance based on hdnDistanceFareCode
                        // Newly added 14.3 Build allow distance edit if SFC_Type = A or F. If SFC_Type = "R" not allow .(Expense group details table)
                        if ($("#hdnDistanceFareCode_" + rCnt).val() != "" && DistanceeditAllow != "A") {
                            $("#txtDistance_" + rCnt).attr("disabled", "disabled");
                        }
                        else {
...
function fnReadIntermediateData() {
    ...
    var distanceFareCode = new Array();
    var distanceFareCode_cur = 0;
    for (var i = 1; i < interRow; i++) {
...
function fnValidateHeader() {
    ...
    if (flag_g.toUpperCase() != "A") {
        if (cpNeed == "YES" || cpNeed == "OPTIONAL") {
            if (cpNeed == "YES") {
                if ($("#CP_No").val() == "") {
                    fnMsgAlert("info", "DCR Header", "Please enter the field 'Beat / Patch Name.");
                    ...
                    if ($("#CP_No").val() != '') {
                        if ($("#hdnCPCode").val() == '' || $("#hdnCPCode").val().length == 0) {
                            fnMsgAlert("info", "DCR Header", "Beat / Patch Name.");
                            fnErrorIndicator("#CP_No");
                            return false;
                        }
                    }
                        ...
                        if (cpJson[0].Category_Name != null && cpJson[0].Category_Name != "") {
                            if (cpJson[0].Region_Code == currentRegion) {
                                if (cpJson[0].Category_Name.toUpperCase() != $("#ddlCategory :selected").text().toUpperCase()) {
                                    fnMsgAlert('info', 'DCR Header', 'The Entered Beat / Patch Name "' + $("#CP_No").val().split('_')[0] + '" does not belong to Work Category ' + $("#ddlCategory :selected").text() + ' . Please check the Beat / Patch Name master for correct Beat / Patch Name.');
    ...
    //    return false;
    //}
    if (!DCRAllowedSpecialCharacters.CheckDCRSpecialCharacterworkplace($("#Work_Place"))) {
        fnMsgAlert('info', 'Information', 'Please remove the special characters in Work Place. <br/> The following characters are only allowed <b>[ _().-@ ]');
        fnErrorIndicator("#Work_Place");
        return false;
    }
    ...
    var Sfc_Setting = true;
    $.each($("#tblActivity .autoactivity"), function (i) {
        Sfc_Setting = false;
        var activityName = $(this).val();
        var lst = $.grep(activityJson_G.Rows, function (v) {
            return (v.Activity_Name + '(' + v.Project_Name + ')') == activityName && v.Setting_Value == 1;
        });
        if (lst.length > 0) {
            Sfc_Setting = true;
            return false;
    ...
    if (Sfc_Setting) {
        ...
        if (distEdit != undefined) {
            if (distEdit.length > 0) {
                var checkedit = distEdit[0].Distance_Edit;
                if (checkedit == "A") {
                    if ($("#ddlCategory :selected").text() == "HQ") {
                        if ($("#hdnDistanceFareCode_1").val() != "") {
                            if (MasterDistance.length > 0) {
                                if ($("#txtDistance_1").val() > parseInt(MasterDistance[0].Distance)) {
                                    fnMsgAlert("info", "DCR Header", "Enter Distance is greater than Actual Distance (" + MasterDistance[0].Distance + ") ");
                                    fnErrorIndicator("#txtDistance_1");
                                    return false;
                                }
                            }
                            else if (popupdistance != "") {
                                if ($("#txtDistance_1").val() > parseInt(popupdistance)) {
                                    fnMsgAlert("info", "DCR Header", "Enter Distance is greater than Actual Distance (" + popupdistance + ") ");
                                    fnErrorIndicator("#txtDistance_1");
                                    return false;
                                }
                            }
                ...
                if (!$("#txtFromPlace_" + i).parent().parent().hasClass("newRow")) {
                    ...
                    if (distEdit != undefined) {
                        if (distEdit.length > 0) {
                            var checkedit = distEdit[0].Distance_Edit;
                            if (checkedit == "A") {
                                if ($("#hdnDistanceFareCode_" + (i - 1)).val() != "") {
                                    if (MasterDistance.length > 0) {
                                        if ($("#txtDistance_" + (i - 1)).val() > parseInt($("#hdnMasterDistance_" + (i - 1)).val())) {
                                            var Masterdis = parseInt($("#hdnMasterDistance_" + (i - 1)).val());
                                            fnMsgAlert("info", "DCR Header", "Enter Distance is greater than Actual Distance (" + Masterdis + ") ");
                                            //$.msgbox("Please enter the field 'Distance'.");
                                            fnErrorIndicator("#txtDistance_" + (i - 1));
                                            return false;
                                        }
                                    }

                                    else if (popupdistance != "") {
                                        if ($("#txtDistance_" + (i - 1)).val() > parseInt(popupdistance)) {
                                            fnMsgAlert("info", "DCR Header", "Enter Distance is greater than Actual Distance (" + popupdistance + ") ");
                                            fnErrorIndicator("#txtDistance_" + (i - 1));
                                            return false;
                                        }
            ...
            if ($.trim($('#txtactivity_' + i.toString()).val()).length > 0) {
                ...
                if ($.trim($('#txtRemarks_' + i.toString()).val()).length > 0) {
                    ...
                    if (!DCRAllowedSpecialCharacters.CheckDCRSpecialCharacterGroup2($("#txtRemarks_" + i))) {
                        ...
                        fnMsgAlert('info', 'Information', 'Please Enter the following characters only <b>[ ' + allowCharacterinAttendatesDCR + ' ]</b> in activity remarks ' + i + '');
                        // fnErrorIndicator("#Work_Place");
                        return false;
                    }
...
function fnSubmitHeader() {
    ...
    if (isTrue) {
        ...
        // work Place
        var workPlace = '';
        var workArea = $('select[name="Work_Place"]').val();
        for (var i = 0; i < workArea.length; i++) {
            workPlace += workArea[i];
            if (i < workArea.length - 1) {
                workPlace += ',';
            }
        }
        ...
        //////// category setting/////
        var ISchecked = $('#categorysetting').prop("checked");
        if (ISchecked == true) {
            ISchecked = 1
        }
        ...
        if (category == "HQ") {
            ...
            if ($("#hdnDistanceFareCode_1").val() != "") {
                ...
                var sfc_a = new Array();
                var sfc = jsonPath(sfc_g, "$.[?((@.Distance_Fare_Code=='" + distanceFareCode + "'))]");
                sfc_a.push(sfc);
                if (!fnCheckSFCCount(sfc_a))
                    return false;
            }
            ...
            if ($("#ddlCategory :selected").text().toUpperCase() == "HQ") {
                ...
                if (intermediateData != "Duplicated") {
                    var sfc_count_check = fnCheckIntermediateSfcCount();
                    if (!sfc_count_check)
                        return false;
                    var travelKm = 0;
                    for (var i = 1; i < interRow; i++) {
                ...
                if ($("#txtAccompanist_" + i).val() != "") {
                    ...
                    switch (i) {
                        case 1:
                            ...
                            acc_g.push({
                                accName: $("#txtAccompanist_1").val(), accCode: $("#hdnAccompanistCode_1").val(), accOnlyDoc: $("#chkOnlyDoctor_1").attr('checked'), accMode: acc1Mode
                            });
                            break;
                        case 2:
                            ...
                            acc_g.push({
                                accName: $("#txtAccompanist_2").val(), accCode: $("#hdnAccompanistCode_2").val(), accOnlyDoc: $("#chkOnlyDoctor_2").attr('checked'), accMode: acc2Mode
                            });
                            break;
                        case 3:
                            ...
                            acc_g.push({
                                accName: $("#txtAccompanist_3").val(), accCode: $("#hdnAccompanistCode_3").val(), accOnlyDoc: $("#chkOnlyDoctor_3").attr('checked'), accMode: acc3Mode
                            });
                            break;
                        case 4:
                            ...
                            acc_g.push({
                                accName: $("#txtAccompanist_4").val(), accCode: $("#hdnAccompanistCode_4").val(), accOnlyDoc: $("#chkOnlyDoctor_4").attr('checked'), accMode: acc4Mode
                            });
                            break;
                        default:
                            break;
        ...
        // Same Route Vaidation 

        var privlege_Sameroute = fnGetPrivilegeValue("REPEAT_SAME_ROUTE_IN_A_DAY", "YES");
        if (privlege_Sameroute == "NO") {
            var arr = [];
            var intermediateTable = $('#tblIntermediate tbody tr');
            if ($('#tblIntermediate tbody tr').length > 1) {
                intermediateTable = $('#tblIntermediate tbody tr:not(:last-child)');
            }
            intermediateTable.map(function (i, e) {
                var obj = {
                };
                obj.Route_Way = $(this).find("[id^=hdnRouteWay]").val();
                if (obj.Route_Way == "R") {
                    obj.From_Place = $(this).find("[id^=txtToPlace]").val();
                    obj.To_Place = $(this).find("[id^=txtFromPlace]").val();
                }
                else {
                    obj.From_Place = $(this).find("[id^=txtFromPlace]").val();
                    obj.To_Place = $(this).find("[id^=txtToPlace]").val();
                }
                obj.Distance_Fare_Code = $(this).find("[id^=hdnDistanceFareCode]").val();
                arr.push(obj);
            })
            ...
            var sameRouteCheck = false;
            var res = "";
            $.ajax({
                type: "POST",
                url: '../HiDoctor_Activity/DCRV4Header/SingleDaySfcValidation',
                //contentType: 'application/json',           
                data: "&data=" + JSON.stringify(objDetails),
                async: false,
                success: function (result) {
                    debugger;
                    if (result != "No Issues") {
        ...
        if (SFCStatus != "") {
            ...
            var _objDetails = CommonDateDetails.fnGetTodaysDateNew();
            console.log(_objDetails);
            try {

                // HD Error Audit Log.
                var objHeaderData = {
                };
                objHeaderData.dcrDate = dcrDate;
                objHeaderData.dcrStatus = dcrStatus;
                objHeaderData.cpCode = JSON.stringify(cpCode);
                objHeaderData.cpName = JSON.stringify(cpName);
                objHeaderData.workPlace = JSON.stringify(workPlace);
                objHeaderData.fromPlace = JSON.stringify(fromPlace);
                objHeaderData.toPlace = JSON.stringify(toPlace);
                objHeaderData.travelMode = travelMode;
                objHeaderData.distance = distance;
                objHeaderData.startTime = startTime;
                objHeaderData.endTime = endTime;
                objHeaderData.distanceFareCode = distanceFareCode;
                objHeaderData.routeWay = routeWay;
                objHeaderData.acc1Name = acc1Name;
                objHeaderData.acc1Type = acc1Type;
                objHeaderData.acc1StartTime = JSON.stringify(acc1StartTime);
                objHeaderData.acc1EndTime = JSON.stringify(acc1EndTime);
                objHeaderData.acc1OnlyDoctor = acc1OnlyDoctor;
                objHeaderData.acc1Mode = acc1Mode;
                objHeaderData.acc2Name = acc2Name;
                objHeaderData.acc2Type = acc2Type;
                objHeaderData.acc2StartTime = JSON.stringify(acc2StartTime);
                objHeaderData.acc2EndTime = JSON.stringify(acc2EndTime);
                objHeaderData.acc2OnlyDoctor = acc2OnlyDoctor;
                objHeaderData.acc2Mode = acc2Mode;
                objHeaderData.acc3Name = acc3Name;
                objHeaderData.acc3Time = JSON.stringify(acc3Time);
                objHeaderData.acc3OnlyDoctor = acc3OnlyDoctor;
                objHeaderData.acc3Mode = acc3Mode;
                objHeaderData.acc4Name = acc4Name;
                objHeaderData.acc4Time = JSON.stringify(acc4Time);
                objHeaderData.acc4OnlyDoctor = acc4OnlyDoctor;
                objHeaderData.acc4Mode = acc4Mode;
                objHeaderData.intermediateData = JSON.stringify(intermediateData);
                objHeaderData.isrcpa = isrcpa;
                objHeaderData.category = JSON.stringify(category);
                objHeaderData.categoryCode = entityCode;
                objHeaderData.activityString = activityString;
                objHeaderData.flag = flag_g;
                objHeaderData.tpDeviation = tpDeviation;
                objHeaderData.cpDeviation = cpDeviation;
                objHeaderData.entryMode = "WEB";
                objHeaderData.dateFrom = dataFrom;
                objHeaderData.sfcRegionCode = sfcRegionCode;
                objHeaderData.sfcVersionNo = sfcVersionNo;
                objHeaderData.sfcCategoryName = sfcCategoryName;
                objHeaderData.dcr_Freeze = DCR_Freeze;
                objHeaderData._objDateDetails = JSON.stringify(_objDetails);
                ...
                var objdata = {
                    ...
                    ModuleName: "DCR",
                    SubModuleName: "DCR Header",
                    ActionTaken: "DCR Header Save",
                    ErrorCode: "",
                    ErrorMessage: "",
                    Data: "POST DATA",
                    Json: JSON.stringify(objHeaderData),
                    SourceOfEntry: "WEB",
                    VersionName: "14.4.3",
                    VersionCode: "14.4.3"
                };
            ...
            $.ajax({
                ...
                "&categoryCode=" + entityCode + "&activityString=" + activityString + "&flag=" + flag_g + "&tpDeviation=" + tpDeviation + "&cpDeviation=" + cpDeviation +
                   "&entryMode=WEB&dateFrom=" + dataFrom + "&sfcRegionCode=" + sfcRegionCode + "&sfcVersionNo=" + sfcVersionNo + "&sfcCategoryName=" + sfcCategoryName + "&dcr_Freeze=" + DCR_Freeze + "&_objDateDetails=" + JSON.stringify(_objDetails) + "&ISchecked=" + ISchecked,
...
function fnRedirectToDoctorVisit(isrcpa) {
    ...
    if (flag_g == "A") {
        ...
        var txtactivity = activityString.split(',');
        var a = [];
        a.push(txtactivity);
        var activitydata = fnGetPrivilegeValue("SHOW_VISIT_SECTION_IN_NONFIELD", "");
        var Privactivity = activitydata.split(',');
        var b = [];
        b.push(Privactivity);
        var disjson = [];
        var docactivity = false;
        if (b[0] != "") {
            for (var i = 0; i < b[0].length; i++) {
                for (var j = 0; j < a[0].length; j++) {
                    if (b[0][i].toUpperCase() == a[0][j].toUpperCase()) {
                        docactivity = true;
                    }
                }
            }
        ...
        if (docactivity == true) {
            ...
            ShowModalPopup('dvLoading');
            $('#main').load('../HiDoctor_Activity/DCRV4DoctorVisit/Index/?status=' + dcrStatus + '&flagRCPA=' + flagRCPA + '&cp=' + escape(cpCode) + '&tp=' + escape(tpCode) + '&dcrActualDate=' + dcrActualDate + '&category=' + escape(category) + '&travelledkms=' + travelKm + '&source=' + sourceString + "&flag=" + flag_g + "&actvity=" + escape(activityString.slice(0, -1)))
        }
        else
...
function fnValidateAccompanist(id) {
            ...
            if (accompanistJson_g != null || !(accompanistJson_g === undefined)) {
                for (var i = 0; i < accompanistJson_g.length; i++) {
                    if ($(id).val() == accompanistJson_g[i].label) {
                        var disjson = $.grep(accompanistJson_g, function (ele, index) {
                            return ele.child > 0 && ele.value == $('#hdnAccompanistCode_' + id.id.split('_')[1]).val();
                        ...
                        else if ($(id).val().split('(')[0].split(',')[1] == "VACANT" || $(id).val().split('(')[0].split(',')[1] == "NOT ASSIGNED") {
                            $("#chkOnlyDoctor_" + (id.id).split('_')[1]).attr('checked', 'checked');
                            $("#chkOnlyDoctor_" + (id.id).split('_')[1]).attr('disabled', true);
                        }
                        else {
    ...
    if (accompanistJson_g != null || !(accompanistJson_g === undefined)) {
        ...
        for (var o = 0; o < accompanistJson_g.length; o++) {
            if (accompanistJson_g[o].label == $(id).val()) {
                i = "true";
                s = accompanistJson_g[o].value;
            }
        }
        if (i == "false") {
            $("#hdnAccompanistCode_" + id.id.split('_')[1]).val('');
        } else {
            $("#hdnAccompanistCode_" + id.id.split('_')[1]).val(s);
        }
    } else {
        $("#hdnAccompanistCode_" + id.id.split('_')[1]).val('');
    }
...
function fnClearHeader() {
    ...
    if (g_DCR_Freeze_Status == "NO") {
        ...
        for (var i = 1; i < accompRow; i++) {
            if ($("#txtAccompanist_" + i).prop('disabled') == false) {
                $("#txtAccompanist_" + i).val("");
                $("#hdnAccompanistCode_" + i).val("");
            }
        }
                    ...
                    if (result.split('$')[1] == "Y") {
                        ...
                        for (var a = 0; a < sfcObj.length; a++) {
                            ...
                            if (sfcObj[a].Is_Route_Complete != "Y") {
                                ...
                                // Change attribute Disabled to Distance based on hdnDistanceFareCode
                                if ($("#hdnDistanceFareCode_" + (a + 1)).val() != "" && DistanceeditAllow != "A") {
                                    $("#txtDistance_" + (a + 1)).attr("disabled", "disabled");
                                }
                                else {
                                    $("#txtDistance_" + (a + 1)).removeAttr("disabled");
                                }
...
function fnBindSelectedSFCCode() {
    ...
    if (currentSFCRow == "Auto") {
        ...
        $("#hdnMasterDistance_Auto").val(selectedVal.split('_')[1]);
        ...
        $("#hdnMasterDistance_" + currentSFCRow).val(selectedVal.split('_')[1]);
        popupdistance = selectedVal.split('_')[1];
        // Change attribute Disabled to Distance based on hdnDistanceFareCode
        if ($("#hdnDistanceFareCode_" + currentSFCRow).val() != "" && DistanceeditAllow != "A") {
            $("#txtDistance_" + currentSFCRow).attr("disabled", "disabled");
        }
        else {
            $("#txtDistance_" + currentSFCRow).removeAttr("disabled");
        }
        ...
        if ($("#" + g_acc_id.id).val().trim() != "") {
            ...
            if (parseInt(chemist_dco_count_g) == 0) {
                if (fnValidateAccompanist(id)) {
                    ...
                    var accomp = fnGetPrivilegeValue("SHOW_ACCOMPANISTS_DATA", "");
                    var accompArr = accomp.split(',');
                    if ($.inArray("SFC", accompArr) > -1) { // check whether the privilege SHOW_ACCOMPANISTS_DATA is mapped with the value "SFC".
                        debugger;
                        fnGetWorkAreas($('#hdnAccompanistCode_' + id.id.split('_')[1]).val());
                    }
                }
            }
            else {
                if (g_acc_id.id.length > 2) {
                    $("#pDel_User_name").text("Dear " + $("#spnUser").text().split('(')[0]);
                    $("#pAcc_name").text($("#" + g_acc_id.id).val().split(',')[1].split('(')[0] + "( " + $("#" + g_acc_id.id).val().split(',')[2].split(')')[0] + ")");
                }
                if (g_acc_id.id != '') {
                    fnValidateAccompanist(id);
                }
...
function fnFreezeDCR() {
    ...
    if (g_DCR_Freeze_Status == "YES") {
        ...
        for (var i = 1; i <= acc_tbl_row_count; i++) {
            if ($("#txtAccompanist_" + i).val() == "") {
                $("#txtAccompanist_" + i).attr("disabled", false);
        ...
        //if (acc_tbl_row_count <= 4) {
        //    tableContent += "<tr>";
        //    tableContent += "<td>" + parseInt(n - 1) + "</td>";
        //    tableContent += "<td style='width: 200px;'><input type='text' id='txtAccompanist_" + parseInt(n - 1) + "'  class='autoAccompanist'  onclick= '$(this).select();'/><input type='hidden' id='hdnAccompanistCode_" + parseInt(n - 1) + "'/><input type='hidden' id='hdnAccompMode_" + parseInt(n - 1) + "'/><span id='popup_" + parseInt(n - 1) + "' class='accPopup'>&nbsp&#62&#62</span></td>";
        //    tableContent += "<td><input type='checkbox' id='chkOnlyDoctor_" + parseInt(n - 1) + "' class='accompchk' />Yes</td>";
        //    tableContent += "<td><input type='text' id='txtStartTime_" + parseInt(n - 1) + "'  class='time accomp' onclick= '$(this).select();' /></td>";
        //    tableContent += "<td><input type='text' id='txtEndTime_" + parseInt(n - 1) + "'  class='time accomp' onclick= '$(this).select();'/></td>";
        //    tableContent += "<td><div id='btnRemoveAccompanist_" + parseInt(n - 1) + "' class='accRemoveIconBlack' ><div class='fa fa-close' onclick='fnRemoveAccompanist(" + parseInt(n - 1) + ");'/></div></td>";
        //    tableContent += "</tr>";
        //    $("#txtAccompanist_" + parseInt(n - 1)).attr("disabled", false);
        //}
        //$("#tblAccompanist tbody").append(tableContent);
        //fnAccompanistEventBinder();
        ...
        if (acc_tbl_row_count != 5)
            acc_tbl_row_count = acc_tbl_row_count - 1;
        $("#tblAccompanist tr:eq(" + acc_tbl_row_count + ")").show();
        $("#txtAccompanist_" + acc_tbl_row_count).attr("disabled", false);
        fnAccompanistEventBinder();
        //SFC Freeze
...
function fnCheckSFCCount(sfc_a) {
    debugger;
    var rValue = true;
    if (fnGetPrivilegeValue("SFC_COUNT_CHECK_IN_DCR", "NO") == "YES") {
        var errorMsg = '';
        if (sfc_a.length > 0) {
            for (var i = 0; i < sfc_a.length; i++) {
                var sfc = sfc_a[i];
                if (sfc[0].SFC_Visit_Count == null || sfc[0].SFC_Visit_Count == '') {
                    sfc[0].SFC_Visit_Count = 0;
                }
                var distanceFareCode = sfc[0].Distance_Fare_Code;
                if (sfc[0].Region_Code == currentRegion && parseInt(sfc[0].SFC_Visit_Count) > 0) {
                    if (distanceFareCode != undefined && distanceFareCode != '') {
                        $.ajax({
                            type: 'POST',
                            url: '../DCRV4Header/CheckSFCCount',
                            data: 'distanceFareCode=' + distanceFareCode + '&dcr_Date=' + dcrDate,
                            async: false,
                            success: function (response) {
                                debugger;
                                var sfc_r = eval('(' + response + ')');
                                if (sfc_r.Tables.length > 0 && sfc_r.Tables[0].Rows.length > 0) {
                                    var sfc_count = sfc_r.Tables[0].Rows[0];
                                    if (sfc_count.Max_Sfc_count <= sfc_count.DCR_SFC_Count) {
                                        if (errorMsg == '') {
                                            errorMsg = "<table><tr><td>From Place</td><td>To Place</td><td> Travel Mode</td></tr>";
                                        }
                                        errorMsg += '<tr><td>' + sfc[0].From_Place + ' </td><td>' + sfc[0].To_Place + '</td><td>' + sfc[0].Travel_Mode + "</td></tr>";
                                    }
                                }
                            }
                        });
        ...
        if (errorMsg.trim() != '') {
            errorMsg += "</table>";
            fnMsgAlert('error', 'DCR Header', 'SFC Visit Count Exceeded for this month <br/>' + errorMsg);
            HideModalPopup('dvLoading');
            rValue = false;
        }
    }
...
function fnCheckIntermediateSfcCount() {
    var distanceFareCode = new Array();
    var sfc_a = new Array();
    var distanceFareCode_cur = 0;
    for (var i = 1; i < interRow; i++) {
        if (!$("#txtFromPlace_" + i).parent().parent().hasClass("newRow")) {
            if ($("#txtFromPlace_" + i).val() != "") {
                distanceFareCode_cur = $("#hdnDistanceFareCode_" + i).val();
                if ((distanceFareCode.indexOf(distanceFareCode_cur)) == -1) {
                    distanceFareCode.push(distanceFareCode_cur);
                    var sfc = jsonPath(sfc_g, "$.[?((@.Distance_Fare_Code=='" + distanceFareCode_cur + "'))]");
                    sfc_a.push(sfc);
                }
            }
        }
...
function fnindependentchk(accompRow, Region_Code) {
    debugger;
    var regioncode
    if (Region_Code == '' || Region_Code == undefined || Region_Code == null) {
        regioncode = $("#hdnAccompanistCode_" + accompRow).val();
    }
    else {
    ...
    $.ajax({
        type: "GET",
        url: '../HiDoctor_Activity/DCRV4Header/GetDoctorCount',
        data: "region_Code=" + regioncode,

        success: function (result) {
            debugger;
            if (result == "0") {
                $("#chkOnlyDoctor_" + accompRow).attr("disabled", true);
            }
            else {
                $("#chkOnlyDoctor_" + accompRow).attr("disabled", false);
            }
        },
        error: function () {
            debugger;
...
function fngetfullindex() {
    debugger;
    $.ajax({
        type: "GET",
        url: '../HiDoctor_Activity/DCRV4Header/GetFullIndex',
        data: "",
...
function fngetCategorySetting(Category_Code) {
    $.ajax({
        type: "GET",
        url: '../HiDoctor_Activity/DCRV4Header/GetCategorySetting',
        data: "Category_Code=" + Category_Code + '&dcrDate=' + dcrDate,

        success: function (result) {
            debugger;
            if (result > 0) {
                $('#categorysettingcheck').html('<input type="checkbox" id="categorysetting"> Return From Work Station');

            }
            else {
                $('#categorysettingcheck').html('');
            }
            if (setting_prefill != "") {
                if (setting_prefill.length > 0) {
                    if (setting_prefill[0].Ischecked == 1) {
                        $('#categorysetting').prop("checked", true);
                    }
                    else {
                        $('#categorysetting').prop("checked", false);
                    }
                }
            }
        }
