var course_test = {
    ...
    constants: { MANDATORY_ERROR_MSG: 'Questions that are  marked * need to be answered. Please answer those questions and then try submitting again' },
    init: function () {
    ...
    pageLoadCall: function (success) {
        ADCourseDetails.insertAdCourseSectionUserExamHeader(course_test.defaults.courseId, course_test.defaults.courseUserAssignmentId,
            ...
            course_test.defaults.sectionId, function () {
                ...
                ADCourseDetails.insertCourseUserExam(examObj, function () {
                    if (success) success();
                }, function () { });
            }, function () { });
    ...
    getQuestionAndAnswers: function () {
        ...
        ADCourseDetails.getQuestionAnswerDetails(course_test.defaults.companyId, course_test.defaults.userId, course_test.defaults.courseId,
    ...
    bindQuestionAnswers: function (data) {
        ...
        if (questions && questions.length > 0) {
            for (var i = 0; i < questions.length; i++) {
                ...
                html += '<div class="question-text">' + curQues.Question_Text + '<span class="spnMand">*</span></div>';
    ...
    bindActions: function () {
        ...
        $('#submit-course-test').unbind('click').bind('click', function () {
            $('#dvAjaxLoad').show();
            //$('#submit-course-test').append("<i class='fa fa-spinner fa-spin'></i>");
            $('#submit-course-test').prop("disabled", true);
            _this.insertCourseResponse();
            $('#submit-course-test').prop("disabled", false);
            return false;
        });
    },
    insertCourseResponse: function () {
        ...
        if (inpArray) {
            ADCourseDetails.insertCourseResponse(inpArray, course_test.defaults.companyId, course_test.defaults.userId, function (data) {
                console.log(data);
                var data = data.split('~');
                if (data[0] == "COMPLETED") {
                    alert('Test completed successfully.');
                    var params = data[3] + "$" + course_test.defaults.publishId + "$" + course_test.defaults.courseId + "$" + course_test.defaults.sectionId + "$" + course_test.defaults.courseUserAssignmentId + "$" + course_test.defaults.courseUserSectionId;
                    var url = 'HDNextGen/AdCourse/ResultPage/' + params;
                    //window.location.href = '/AdCourse/ResultPage/?examId=' + data[3] + '&publishId=' + course_test.defaults.publishId +
                    ...
                    course_test.loadResultPage(url);
                } else {
                    alert(data[2]);
                    $('#dvAjaxLoad').hide();
                }
            }, function () { $('#dvAjaxLoad').hide(); });
        }
        $('#submit-course-test').prop("disabled", false);
    },
    loadResultPage: function (url) {
        $('#main').load(url);
        //$('#dvAjaxLoad').show();
    },
    prepareAnswerObject: function () {
        ...
        if (hdr && hdr.length > 0 && det.detailObj && det.quesObtj) {
            var courseObj = {};
    ...
    getByQuestionTypeOne: function (i, questionId) {
        ...
        if (chkActive.length > 0) {
            ...
            assessmentDetailObj.Count_of_User_Answers = chkActive.length;
            assessmentDetailObj.Count_Of_User_Correct_Answers = userCorrectAnswerCount;
            assessmentDetailObj.Course_User_Assignment_Id = course_test.defaults.courseUserAssignmentId;
            assessmentDetailObj.Couse_User_Section_Mapping_Id = course_test.defaults.courseUserSectionId;
            console.log(userCorrectAnswerCount + ', ' + count);
            if (chkActive.length !== userCorrectAnswerCount) {
                assessmentDetailObj.Is_Correct = false;
            } else {
