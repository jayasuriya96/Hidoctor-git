        ...
        success: function (jsData) {
            if (jsData != '') {
                ...
                if (!(dailyPlan.Rows === undefined) && dailyPlan.Rows.length > 0) {
                    ...
                    tblCont += "<th>CP Name : " + dailyPlan.Rows[0]["CP_Name"] + "</th>";
...
function fnLoadAttendanceCreate() {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result != "") {
                var dateFormate = $("#hdnDCRDateCreate").val().split('-')[2] + '/' + $("#hdnDCRDateCreate").val().split('-')[1] + '/' + $("#hdnDCRDateCreate").val().split('-')[0];
                tblCont = "<div ><h3 style='width: 100%;margin:0px auto;padding-top: 20px;'>You have attendance on " + dateFormate + ".(" + result + ")</h3></div>";
                $("#divDailyCallHeaderCreate").html(tblCont);
                return;
            }
        }
