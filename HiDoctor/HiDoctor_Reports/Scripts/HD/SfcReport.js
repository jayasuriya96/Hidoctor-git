...
//To check SFC validity single check
function AllChecked() {
    if ($('.clsValidCheck:checked').length == 3) {
        
        $('#chkValidAll').attr('checked', 'checked');
    }
    else {
        $('#chkValidAll').attr('checked', false);
    }
}
...
//For All View
function fnGetsfcReportforAllView() {
    if ($('#optViewInScreen').attr('checked') == "checked") {
        fnGetSFCReportinHTMLFormatforAllview();
    }
    else if ($('#optExportToExcel').attr('checked') == "checked") {
        fnGetsfcReportIntoExcelReportforAllView();
    }
}

//Get SFC Report for All View
function fnGetSFCReportinHTMLFormatforAllview() {
    var regionCodes = "";
