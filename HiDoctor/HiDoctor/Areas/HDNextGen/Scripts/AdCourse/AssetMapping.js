...
var assetMapping = {    
    ...
    getRequiredAssets: function (assets) {
        var arAssets = new Array();
        assets = JSON.parse(assets);
        for (var i = 0; i < assets.length; i++) {
            var curAsset = assets[i];
            var lastIndexOfSlash = curAsset.onlineURL.lastIndexOf('/');
            var ext = curAsset.onlineURL.substr(lastIndexOfSlash + 1, curAsset.onlineURL.length);

            ext = ext.substr(ext.lastIndexOf('.') + 1, ext.length).toLowerCase();
            if (ext != 'xls' && ext != 'xlsx' && ext != 'zip') {
                arAssets.push(curAsset);
            }
        }
        return JSON.stringify(arAssets);
    },
    ...
    fnGetCourseAssetMappings: function (success) {        
        ...
        var postData = {};
        postData.name = "utcOffset";
        postData.value = commonValues.getUTCOffset()
        postAry.push(postData);
        
        DPAjax.requestInvoke('AdCourse', 'GetAssetsForBrowse', postAry, "GET", function (data) {
            data = assetMapping.getRequiredAssets(data);
            _this.fnGetMetatags(data, function (data1) {
                ...
                if (data1 == null || data1.coreAssets == null ||
                    ...
                    $("#main").load("HDNextGen/AssetUpload/AssetMaster");
                    return;
                }
                if (data1 != null) {
                    if (data1.metaTags)
                ...
                $('#btnBackCreateCourse').bind('click', function (e) {                    
                    var courseDetail = _this.courseId + "^" + encodeURIComponent(_this.courseName)
                    $("#main").load("/HDNextGen/AdCourse/SectionHome/" + courseDetail);
                });
    ...
    fnInsertAssetMapping: function (elem) {
        ...
        DPAjax.requestInvoke('AdCourse', 'InsertCourseAssetMapping', arData, "POST", function (data) {
            if (data != null) {
                ...
                var courseDetails = sectionId + "^" + courseId + "^" + encodeURIComponent(courseName) + "^" + encodeURIComponent($('#txtSectionName').val());
                $("#main").load("/HDNextGen/AdCourse/CreateQuestion/" + courseDetails);
            }
            else {
            ...
            for (var i = 0; i <= assets.length - 1; i++) {
                if (_this.selectedAssets[assets[i].daCode] == null) {
                    ...
                    btnGroup.bind('click', function (e) {
                        ...
                        //player.show();
                        assetMapping.previewAsset($(_this).data('asset'));
                    });
    ...
    previewAsset: function (result) {
        Player.CoreREST._defaultServer = "/";
        //Global Variable - HDNG_gSubdomain, HDNG_gCompanyId
        Player.Services.defaults.subdomainName = HDNG_gSubdomain;
        Player.Services.defaults.companyId = HDNG_gCompanyId;
        Player.getAssetImagesAlt([result], "daCode", "DA_Code", function (assets) {
            var player = new Player({
                headerLogo: "/Images/hi-doctor-logo.png",
                assets: assets,
