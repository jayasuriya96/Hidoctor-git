...
var COURSE = {
    ...
    imageSizeExceed: function (size) {
        //return 'Please upload file size less than ' + (size / 1024) + ' KB' FOR KB
        return 'Please upload file size less than ' + (size / 1024 / 1024) + ' MB'
    },
    selectedAssets: {},
    metaTagDataAssets: null,
    fnCheckCourseNameExists: function () {
        var crName = $("#txtCourseName").val();
        var crId = $("#hdnCourseId").val();
        var arData = new Array();
        var course = {};
        course.Course_Id = crId;
        course.Course_Name = crName;
        ...
        //$('#spnCourseNameErr').html("");
        DPAjax.requestInvoke("AdCourse", "CheckAdCourseNameExists", arData, "POST",
            function (result) {                
                if (result > 0) {
                    $('#dvCourseNameValid').html('<i class="fa fa-times" ></i>');
                    $('#dvCourseNameValid').removeClass('clsValid');
                    $('#dvCourseNameValid').addClass('clsInvalid');
                    $('#spnCourseNameErr').html("Course Name already exists");
                }
                else {
                    $('#dvCourseNameValid').html('<i class="fa fa-check"></i>');
                    $('#dvCourseNameValid').removeClass('clsInvalid');
                    $('#dvCourseNameValid').addClass('clsValid');
                }
            },
            function (e) {                
            });
    ...
    fnInitializeEvents: function () {
        ...
        $('#txtCourseName').blur(function () {            
            ...
            if (flag) {                
                    COURSE.fnCheckCourseNameExists();
            }
        });
                   ...
                   arFileName[0].toUpperCase() == "BMP" || arFileName[0].toUpperCase() == "GIF") {
                    if (input.files[0].size <= COURSE.maxImageSize) {
                        ...
                        filerdr.onload = function (e) {
                            ...
                            $('#dvSurImagePreview').html("<img alt='' class='thumbnail' src='" + e.target.result + "'/>");
    ...
    fnCourseSubmit: function () {
        ...
        if (flag) {
            ...
            COURSE.fnCourseSubmitAction(arData, courseJson, fileName);
    ...
    fnCourseSubmitAction: function (arData, courseJson, fileName) {       
        if (fileName != '' && fileName != null && fileName != undefined) {          

            BeginFileUpload($('#course-form'), "txtCourseImage", fileName, "AdCourse", "ChunkFileUpload",
                function successcallback(data) {
  
                    if (data.type == "SUCCESS") {
           

                        courseJson.value[0].Course_Image_URL = data.url;                        
                        arData.splice(0, 0, courseJson);                        
            ...
            courseJson.value[0].Course_Image_URL = "";
            arData.splice(0, 0, courseJson);
            console.log(arData);
            DPAjax.requestInvoke("AdCourse", "InsertCourse", arData, "POST",
            function (result) {
                var arResult = result.split(resultDelimiter);
                if (arResult[0] == "SUCCESS") {
                    COURSE.fncourseImgUpoadSuccess("", result);
                }
                else {
                    $('#btnCreateCourse').show();
                    COURSE.courseSubmitSuccess(result, '');
    ...
    courseSubmitSuccess: function (result, mode) {
        ...
        if (result != null) {
            ...
            if (spl.length >= 3) {                
                ...
                var param1 = {};
                param1.name = 'courseName';
                param1.value = courseName;
                nameArry.push(param1);
                                
                var courseVal = courseId + '^' + encodeURIComponent(courseName);
                $("#main").load('/HDNextGen/AdCourse/SectionHome/' + courseVal);                
            }            
            else {
                alert('Error occured');
            }
                ...
                if (jsonData.length > 0) {                                 
                  ...
                  //  $('#txtCourseInstruct').val(jsonData[0].Course_Instruction);
                    if (jsonData[0].Course_Image_URL != null && jsonData[0].Course_Image_URL != '' && jsonData[0].Course_Image_URL != undefined) {                       
                        $('#dvSurImagePreview').empty().append('<img alt="" class="preview-thumb thumbnail" src=""/>');
