var resultPage = {
    ...
    getQuestionAnswerDetails: function () {
        ADCourseDetails.getAdSectionQuestionDetails(resultPage.defaults.examId, resultPage.onGotResultDetail, function () { });
    },
    onGotResultDetail: function (data) {
        ...
        $('#retake-test').bind('click', function (e) {
            $('#dvAjaxLoad').show();
            var params = resultPage.defaults.courseId + "$" + resultPage.defaults.sectionId + "$" + resultPage.defaults.publishId + "$" + resultPage.defaults.courseUserAssignmentId + "$" + resultPage.defaults.courseUserSectionId;
            ...
            var url = 'HDNextGen/AdCourse/QuestionAnswers/' + params;
            resultPage.loadQuestionAndAnswer(url);
            return false;
        });
        $('#dvAjaxLoad').hide();
    },
    loadQuestionAndAnswer: function (url) {        
        $('#main').load(url, function (e) {
            var defaults = {
                userId: userId, companyId: companyId, courseId: courseId, publishId: publishId,
                sectionId: sectionId, courseUserAssignmentId: courseUserAssignmentId, courseUserSectionId: courseUserSectionId
            };

            $.extend(course_test.defaults, defaults);
            course_test.init();
            $('#dvAjaxLoad').hide();
        });
    },
