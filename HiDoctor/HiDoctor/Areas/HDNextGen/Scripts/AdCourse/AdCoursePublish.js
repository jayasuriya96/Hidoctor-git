var COURSE = {
    ...
    CompanyId_g: HDNG_gCompanyId,
    UserId_g : HDNG_gUserId,
            ...
            success: function (jsData) {
                if (jsData != null) {
                    ...
                    if (courseList.length > 0) {
                        ...
                        //courseContent += "<div class='col-md-10'>Report</div>";
                        courseContent += "</div>";
                    ...
                    if (sectionList.length > 0) {
                        ...
                        for (var i = 0; i < sectionList.length; i++) {
                            ...
                            sectionContent += "<td><a href='#' onclick='return COURSE.fnSelectAssets(" + COURSE.CompanyId_g + "," + sectionDetail.Course_Id + "," + sectionDetail.Section_Id + ", \"" + sectionDetail.Section_Name + "\");' >" + sectionList[i].No_Of_Assets_Mapped + "</a></td>";
                            sectionContent += "<td><a href='#' onclick='return COURSE.fnSelectQuestions(" + COURSE.CompanyId_g
                                + ", " + COURSE.UserId_g + ", " + sectionDetail.Course_Id
                        ...
                        for (var i = 0; i < sectionList.length; i++) {
                            ...
                            if (sectionList[i].No_Of_Questions_Mapped > 1) {
                                assessmentContent += "<td style='width: 30%;'><input type='text' id='txtSection_" + a + "' maxlength='3' />";
                            }
                            else {
                                assessmentContent += "<td style='width: 30%;'><input type='text' id='txtSection_" + a + "' disabled='disabled' value='100' />";
                            }
    ...
    fnBindUserType: function () {
        $.ajax({
            type: "POST",
            url: '/AdCourse/GetUserTypeDetails',
            data: "D",
            success: function (jsData) {
                if (jsData != null) {
                    jsData = eval(jsData);
    ...
    fnBindUserDetails: function (userTypeCode) {
        ...
        $.ajax({
            type: "POST",
            url: '/AdCourse/GetEmployeeDetails',
    ...
    fnPublishCourseSubValidate: function () {
        ...
        if ($('#txtValidFrom').val() == '') {
            debugger;
            $('#spnValidFromErr').html(PublidhFromDateError);
            flag = false;
        }

        if ($('#txtValidTill').val() == '') {
            debugger;
            $('#spnValidTillErr').html(PublishToDateError);
            flag = false;
        }
    ...
    fnPublishCourse: function (courseId_g) {
        ...
        if (flag) {
            ...
            DPAjax.requestInvoke("AdCourse", "CheckPublishNameExists", arData, "POST",
               function (result) {
                   if (result == 0) {
                       //To check if PublishName Exists
                       DPAjax.requestInvoke("AdCourse", "InsertPublishCourseDetails", arData, "POST",
                       function (result) {
                           if (result != "" && result != null) {
                               alert("Course Published Successfully");
                               $("#main").load('/HDNextGen/AdCourse/AdCourseList');
                       ...
                       btnSubmit.find('i').remove();
                       alert("Publish Name already exists");
                       return false;
                   }
               },
               function (e) {
    ...
    fnSelectAssets: function (companyId, courseId, sectionId, sectionName) {
        ADCourseDetails.getAdAssetsByCourseId(companyId, courseId, sectionId, function (data) {
    ...
    fnSelectQuestions: function (companyId, userId, courseId, sectionId, publishId, sectionName) {
        //ADCourseDetails.getAdCourseQuestionAnswerDetails(companyId, userId, courseId, sectionId, publishId, function (data) {
        ADCourseDetails.getAdCourseQuestionAnswerDetails(companyId, userId, courseId, sectionId, publishId, function (data) {
    ...
    initPopups: function () {
        $('#dvAssetsPopup .button a').unbind().bind('click', function (e) {
            HideModalPopup("dvAssetsPopup");
            return false;
        });
        $('#dvQuestionsPopup .button a').unbind().bind('click', function (e) {
