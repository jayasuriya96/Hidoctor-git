...
var DieticianAPPLanding = {
    ...
    fncheckcombinationSuccessCallback: function (response) {
        ...
        if (response != null && response.list.length > 0) {
            ...
            //for (var j = 0; j < $(".dvMainBody form").length - 1; j++) {
            //    if ($("#ddluser" + (j + 1) + '_' + check[1] + "").val() == $("#ddluser" + (j + 2) + '_' + check[1] + "").val() && $("#ddlActivity" + (j + 1) + '_' + check[1] + "").val() == $("#ddlActivity" + (j + 2) + '_' + check[1] + "").val() && $("#ddlsubActivity" + (j + 1) + '_' + check[1] + "").val() == $("#ddlsubActivity" + (j + 2) + '_' + check[1] + "").val()) {
            //        swal({
            //            icon: "info",
            //            title: "Info",
            //            text: "There is already camp for this combination.",
            //            button: "Ok",
            //        });
            //        $("#ddluser" + (j + 2) + '_' + check[1] + "").val(response.list[j+2].For_Region_Code);
            //        $("#ddlActivity" + (j + 2) + '_' + check[1] + "").val(response.list[j + 2].Camp_Type);
            //        $("#ddlsubActivity" + (j + 2) + '_' + check[1] + "").val(response.list[j + 2].Camp_Sub_Type);
            //        $("#go" + Check_g + "").hide();
            //        $("#view" + Check_g + "").show();
            //    }
            //}
    ...
    fngetstatusSuccessCallback: function (response) {
        ...
        if (response != null && response.list.length > 0) {
            for (var i = 0; i < response.list.length; i++) {
                ...
                content += '<input type="text" id="ddluser' + 1 + '_' + acc_g + '" name="ddluser' + 1 + '_' + acc_g + '" class="form-control" maxlength="" />';
                content += '</div>';
                content += '</div>';
                content += '</div>';
                content += '<div class="form-group row">';
    ...
    fngetusersSuccessCallback: function (response) {
        ...
        if (response != null && response.list.length > 0) {
            ...
            Users_g = data1;
            $('#dvusers' + User_g + '').empty();
            $('#dvusers' + User_g + '').html('<input type="text" id="ddluser' + User_g + '" name="ddluser' + User_g + '" class="form-control" maxlength="" />');
            ...
            Users.appendTo('#ddluser' + User_g + '');
            var regioncode = RegionCode;
            var msObject = document.getElementById("ddluser" + User_g + "").ej2_instances[0];
    ...
    fnredirect: function (v, k, headerid) {
        if ($('select[name="ddluser' + v + '_' + k + '"]').val() == null || $('select[name="ddluser' + v + '_' + k + '"]').val() == "") {
            swal({
                icon: "info",
                title: "Info",
        ...
        var selectedregionname = $('select[name="ddluser' + v + '_' + k + '"]').val();
        if (resp_g.list.length > 0) {
            // if (resp_g.list[v-1].status == 1) {
            if (resp_g.list[v - 1].For_Region_Code != $('select[name="ddluser' + v + '_' + k + '"]').val() || resp_g.list[v - 1].Camp_Type != $("#ddlActivity" + v + '_' + k + "").val() || resp_g.list[v - 1].Camp_Sub_Type != $("#ddlsubActivity" + v + '_' + k + "").val()) {
                var headerid = 0;
            }
            // }
        }
    ...
    fncheckstatus: function (v, k, headerid) {
        ...
        var a = $("#dcrdate" + k + "").text().split('/')
        var dcrdate = a[2] + '-' + a[1] + '-' + a[0];
        Method_params = ["DieticianReporting/checkstatus", DieticianAPPLanding.defaults.companyCode, dcrdate, UserCode, $('select[name="ddluser' + a_g + '"]').val(), $("#ddlActivity" + a_g + "").val(), $("#ddlsubActivity" + a_g + "").val()];
    ...
    fncheckstatusSuccessCallback: function (response) {
        ...
        var selectedregionname = $('select[name="ddluser' + a_g + '"]').val() //$("#ddluser" + a_g + "").val();
        if (response == 1) {
            swal({
                icon: "info",
                title: "Info",
